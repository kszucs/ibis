...
class IfNull(ValueOp):
    ...
    # we may not need this if we use this rule on valueop by default
...
class NullIf(ValueOp):
    ...
    output_dtype = rlz.dtype_like(
        'args'
    )  # TODO(kszucs): should be just `arg`?
...
class StructField(ValueOp):
    ...
    @property
    def index(self):
        field_names = self.arg.type().names
        return field_names.index(self.field)
...
class DistinctColumn(ValueOp):
    ...
    # TODO(kszucs): by subclassing unaryop we wouldn't need to define the
    # output rules
    output_dtype = rlz.dtype_like("arg")
    output_shape = rlz.shape_like("arg")
