...
def compile_aggregation(t, op, **kwargs):
    ...
    if op.predicates:
        src_table = src_table.filter(
            t.translate(
                functools.reduce(operator.and_, op.predicates), **kwargs
            )
        )
    ...
    aggs = [t.translate(m, context=context, **kwargs) for m in op.metrics]
...
def compile_aggregator(t, op, *, fn, context=None, **kwargs):
...
@compiles(ops.Any)
def compile_any(t, op, context=None, **kwargs):
    return compile_aggregator(t, op, fn=F.max, context=context, **kwargs)


@compiles(ops.NotAny)
def compile_notany(t, op, context=None, **kwargs):
    ...
    if context is None:
        ...
        return compile_aggregator(t, op, fn=fn, context=context, **kwargs)
    else:
        return ~compile_any(t, op, context=context, **kwargs)
...
@compiles(ops.NotAll)
def compile_notall(t, op, context=None, **kwargs):
    ...
    if context is None:
        ...
        return ~compile_all(t, op, context=context, **kwargs)
...
def compile_window_op(t, op, **kwargs):
    ...
    result = t.translate(operand, **kwargs, context=context).over(
        pyspark_window
    )
...
def compile_reduction_udf(t, op, context=None, **kwargs):
    spark_output_type = spark_dtype(op.return_type)
